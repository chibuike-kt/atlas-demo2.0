...
class ExecutionController extends Controller
{
     ...
     *
     * Security order:
     *   1. Ownership     — rule belongs to this user
     *   2. Active check  — rule must be enabled
     *   3. Idempotency   — return cached result if key already processed
     *   4. Lock          — prevent parallel execution of same rule
     *   5. Velocity      — fraud rate limiting
     *   6. Execute       — engine runs with step retry
     *   7. Record        — save idempotency result for future retries
     */
    public function execute(Request $request, string $id): JsonResponse
    {
        ...
        // If absent we generate one from user+rule+current-minute — safe for manual triggers.
        $idempotencyKey = $request->header('X-Idempotency-Key')
            ?? hash('sha256', "{$user->id}:{$rule->id}:" . date('YmdHi'));

        $existingId = $this->idempotency->getExistingResult($user->id, $rule->id, $idempotencyKey);
        if ($existingId) {
            $existing = RuleExecution::with('steps')->find($existingId);
            if ($existing) {
                return response()->json([
                    'success'    => true,
                    'idempotent' => true,
        ...
        // 4. Execution lock — prevents parallel runs of same rule
        $lock = null;
        try {
        ...
        try {
            ...
            // Bust cached average so next anomaly check is fresh
    ...
    {
        $history = app(\App\Services\LedgerService::class)->history(auth()->id());
